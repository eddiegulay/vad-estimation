...
CONFIG_PATH = REPO_ROOT / "configs" / "model" / "crnn_v1.yaml"
...
def test_frontend_dft_basis_is_not_trainable():
    ...
    # buffers aren't counted in .parameters() (identity check -- tensor `in`
    # would otherwise fall back to elementwise `==`, which errors on shape mismatch)
...
def test_streaming_matches_full_forward():
    """Core Phase 5 gate: chunked incremental calls must reproduce the
    full-sequence forward within float tolerance (plan §6).
    """
    model, config = _load_model()
    model.eval()
    chunk_samples = config["frontend"]["chunk_samples"]
    ...
    with torch.no_grad():
        ...
        streaming_probs = torch.stack(streaming_probs, dim=1)  # [B, N]
